    ...
    <header className="w-full">
      ...
      <div className="bg-ivory border-b border-slate-100 text-sm text-mutedGoldGray">
        <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between">
          <div className="text-charcoalText/80">شحن مجاني للطلبات الأكبر من 300 جنيه — توصيل سريع وفاخر</div>
      ...
      <nav className="sticky top-0 z-50 glass backdrop-blur-md border-b border-slate-200/10">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          {/* Left: Brand (RTL) -> visually placed right */}
          <div className="flex items-center gap-4">
            <Link href="/" className="flex items-center gap-3">
              <div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-full border border-[#e9d79e] bg-white shadow-[0_6px_20px_rgba(212,175,55,0.18)]">
                <img src="/hoor-logo.jfif" alt="شعار متجر حور" className="h-full w-full object-cover" />
              </div>
              <div className="leading-tight">
                <div className="text-charcoalText font-extrabold text-lg">حور</div>
                <div className="text-gold text-xs">HOOR</div>
              </div>
            </Link>

            {/* Categories - hidden on xs */}
            <ul className="hidden lg:flex items-center gap-2 mx-6">
              {categories.map((c) => (
                <li key={c.href} className="text-sm text-charcoalText/80 hover:text-charcoalText/100 px-3 py-2 rounded-md transition">
          ...
          <div className="flex-1 px-4 max-w-2xl">
            <div className="relative">
              ...
              <div className="flex items-center bg-white rounded-full shadow-sm border border-slate-100 overflow-hidden">
                <input
                  ref={inputRef}
                  id="search"
                  value={query}
                  ...
                  className="flex-1 px-4 py-3 text-sm placeholder:text-slate-400 rtl:pr-4 ltr:pl-4 focus:outline-none"
                  aria-label="بحث عن العطور"
                />
                <button
                  aria-label="ابحث"
                  className="px-4 py-3 border-l border-slate-100 text-slate-600"
                >
                  <Search size={16} />
                </button>
              </div>
              ...
              {showSuggestions && suggestions.length > 0 && (
                <div className="absolute right-0 left-0 mt-2 bg-white rounded-lg shadow-lg border overflow-hidden z-40">
                  <ul>
                    {suggestions.map((s) => (
                      <li key={s} className="px-4 py-3 hover:bg-slate-50 cursor-pointer text-sm">
                        {s}
                      </li>
                    ))}
                  </ul>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <Link href="/advisor" className="hidden sm:inline-flex items-center gap-2 rounded-full border border-[#efe2c2] bg-[#fffaf0] px-3 py-2 text-sm font-medium text-[#3a2b12] btn-hover-bounce transition-all duration-300">
              <Filter size={16} className="icon-shadow" />
              <span>مستشار العطور</span>
            </Link>

            <Link href="/support" className="hidden rounded-full border border-[#efe2c2] bg-[#fffaf0] px-3 py-2 text-xs font-medium text-[#3a2b12] btn-hover-bounce transition-all duration-300 md:inline-flex">
              خدمة العملاء
            </Link>

            <Link href="/favorites" className="relative rounded-full p-2.5 text-[#1f2937] icon-shadow transition-all duration-300 hover:bg-slate-100">
            ...
            <button
              ...
              onClick={openCart}
              className="relative rounded-full p-2.5 text-[#1f2937] icon-shadow transition-all duration-300 hover:bg-slate-100"
            >
            ...
            {user ? (
              <div className="flex items-center gap-2 rounded-full border border-[#eadab0] bg-[#fffdf8] px-2 py-1.5 shadow-sm">
              ...
              <Link href="/auth/login" className="flex items-center gap-2 rounded-full bg-[#d4af37] px-4 py-2 text-sm font-bold text-white shadow-md btn-hover-bounce transition-all duration-300">
                <User size={16} className="icon-shadow" />
                تسجيل الدخول
              </Link>
            )}
          </div>
