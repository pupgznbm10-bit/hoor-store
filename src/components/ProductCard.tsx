...
export type Product = {
  ...
  fragranceFamily?: string; // e.g., 'عودي', 'أورينتال'
...
/**
 * ProductCard: Reusable perfume card component.
...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <article className="card-hover group overflow-hidden rounded-[26px] border border-[#efe5d4] bg-white shadow-[0_20px_45px_rgba(14,17,28,0.08)]">
      <div className="relative aspect-[4/5] overflow-hidden bg-[#f6f0e7]">
        ...
        <div className="absolute inset-x-3 top-3 flex items-start justify-between">
          <div className="flex flex-col gap-2">
            {product.originalPrice && (
              <span className="rounded-full bg-red-600 px-2.5 py-1 text-[10px] font-bold text-white shadow-sm glow-effect">تخفيض</span>
            )}
          ...
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-full border border-white/70 bg-white/85 text-[#1d1d1d] shadow-sm backdrop-blur-sm icon-shadow transition"
        ...
        <div className="absolute inset-x-3 bottom-3 flex items-center justify-between gap-2 opacity-0 transition duration-300 group-hover:opacity-100">
          <div className="flex flex-wrap gap-2">
            {product.volumes?.slice(0,2).map((v) => (
              <button
                ...
                className="rounded-full bg-white/90 px-3 py-1.5 text-[11px] font-medium text-[#1a1a1a] shadow-sm btn-hover-bounce transition"
              >
                {v}
              </button>
            ))}
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-full bg-[#d4af37] px-3 py-2 text-sm font-bold text-white shadow-md btn-hover-bounce transition"
          >
            <Plus size={14} className="icon-shadow" />
            إضافة
          </button>
        </div>
      </div>
      ...
      <div className="space-y-3 p-4">
        <div className="flex items-start justify-between gap-2">
          <div>
            <Link href={`/products/${product.id}`} className="text-base font-bold text-[#171717] hover:text-[#a67c00] transition">
              {productName}
            </Link>
            {product.name_en && <p className="text-xs text-[#6b7280]">{product.name_en}</p>}
          </div>
          <div className="text-left">
            <div className="text-base font-black text-[#111827] gold-text-gradient">{product.price.toLocaleString()} ج.م</div>
            {product.originalPrice && (
        ...
        <Link
          ...
          className="inline-flex items-center gap-2 text-sm font-semibold text-[#865b10] hover:text-[#6b4a00] icon-shadow transition"
        >
          تفاصيل المنتج
          <ArrowLeft size={14} />
        </Link>
