...
export default function ProductDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  const noteSections = {
    top: Array.isArray(product.notes?.top) ? product.notes.top : Array.isArray(product.notes) ? product.notes.slice(0, 2) : [],
    heart: Array.isArray(product.notes?.heart) ? product.notes.heart : Array.isArray(product.notes) ? product.notes.slice(2, 4) : [],
    base: Array.isArray(product.notes?.base) ? product.notes.base : Array.isArray(product.notes) ? product.notes.slice(4, 6) : [],
  };
    ...
    <div className="mx-auto max-w-6xl px-4 py-10">
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
            ...
            <div className="mt-4 flex gap-3">
              {productImages.map((img: string, idx: number) => (
                <button
                  ...
                  className={`h-20 w-20 overflow-hidden rounded-2xl border ${idx === selectedImage ? 'border-[#d4af37] ring-2 ring-[#d4af37]/30' : 'border-slate-200'}`}
                >
                  <img
                    src={img}
                    alt={`${product.name ?? 'منتج'} ${idx}`}
                    className="h-full w-full object-cover"
                    onError={(e) => { (e.currentTarget as HTMLImageElement).src = fallbackImage; }}
                  />
                </button>
        ...
        <div>
          ...
          <h1 className="mt-2 text-3xl font-black text-[#111827]">{product.name ?? 'منتج بدون اسم'}</h1>
          <p className="mt-2 text-sm text-slate-500">{product.brand ?? 'Hoor'}</p>
          <p className="mt-4 text-sm leading-7 text-slate-600">{product.description ?? 'لا توجد تفاصيل متاحة لهذا المنتج في الوقت الحالي.'}</p>
              ...
              {[
                ...
                <div key={label as string}>
                  ...
                  <ul className="mt-2 space-y-1 text-slate-600">
                    {(notes as string[]).length > 0 ? (notes as string[]).map((n: string) => <li key={n}>{n}</li>) : <li className="text-slate-400">غير متوفر</li>}
                  </ul>
                </div>
              ))}
            </div>
          ...
          <div className="mt-6 space-y-4">
            <div>
              <div className="mb-1 flex items-center justify-between text-sm text-slate-600">
                <span>الثبات</span>
                <span className="font-bold text-[#111827]">{rating}/5</span>
              </div>
              <div className="h-2.5 overflow-hidden rounded-full bg-slate-100">
                <div className="h-full rounded-full bg-[#d4af37]" style={{ width: `${(rating / 5) * 100}%` }} />
              </div>
            </div>
          </div>
          ...
          <div className="mt-6 flex flex-wrap items-center gap-4">
            ...
            <div className="text-2xl font-black text-[#111827]">{price.toLocaleString()} ج.م</div>
          </div>

          <div className="mt-6 flex flex-wrap gap-3">
            {(product.volumes && product.volumes.length > 0 ? product.volumes : ['50ml']).map((v: string) => (
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <button
              ...
              className="rounded-full bg-[#d4af37] px-6 py-3 text-sm font-bold text-white shadow-md transition hover:bg-[#c79d1e]"
            >
              إضافة إلى السلة
            </button>
            <button
